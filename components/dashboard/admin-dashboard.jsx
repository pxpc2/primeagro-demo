...
export default function AdminDashboard({ clientes }) {
  ...
  const handleConfirmDelete = () => {
    handleDeleteEnquadramento(selectedUserId);
  ...
  const handleDeleteEnquadramento = async (userId) => {
    ...
    setDeletarEnquadramentoStatus((prevState) => ({
      ...
      [userId]: {
        ...
        loading: false,
        success: false,
        buttonText: deletado ? "Enquadramento deletado!" : "Deleção falhou",
        failed: !deletado,
      },
    }));
  };
              ...
              >
                {status.loading ? (
                  ...
                  <>
                    ...
                    {status.success && <CheckIcon className="ml-1.5 mt-0.5" />}
                  </>
                )}
              </Button>
            </div>
      ...
      <Dialog open={deletarDialogOpen} onOpenChange={setDeletarDialogOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você tem certeza que deseja deletar este enquadramento? Esta ação
              não pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDeletarDialogOpen(false)}>
              Cancelar
            </Button>
